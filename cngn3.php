<?php
    ...
    class CNGN {
        ...
        */
        ...
        public function bitcoin(string $btc_csv, int $day_cnt, $data_column = 1, $date_column = 0)
        {
            $sf = fopen("$btc_csv","r");
            $seq = [];
            $data = [];
        ...
        public function stock_csv(string $stock_csv, int $day_cnt = 1)
        {
            $sf = fopen("$stock_csv","r");
            $seq = [];
            $data = [];
            fgets($sf);
            $day_before = 0;
            ...
            {
                $date_1 = $day_before;
                $seq[] = [ ($y), $date_1, $day_before, $data[0] ];
                $day_before = $data[4];
                $y+=$day_cnt;
            }
            fclose($sf);
            ...
            $string = "<table>";
            $string .= "<tr><td>Long Form Date </td><td> Integrand </td><td> Differential </td><td> Differential </td><td> Integral </td><td> Price</td><td>Low</td></tr>";
            $y = 0;
            $offset = 1;
            $vals = [];
            $x = 0;
            foreach($seq as $key) {
                ...
                $vals[] = $this->integrand($key);
                $c = $this->differential($key);
                $string .= "<tr><td>".$key[3]." </td><td> ".$vals[3]." </td><td> $c  </td><td>". $this->derive($vals) . " </td><td> ".$this->integral($key)."</td><td>".$key[1]."</td>";
                $lo = $this->derive($vals) / $vals[3] / $c * $this->derive($vals);
                while ($lo <= 0.999)
                    $lo *= 10;
                $short_low = abs(($lo / 10) - 1);
                $short_low = ($lo * $vals[2]);
                $short_low = round($short_low / 100 / 2,2);
                
                #if ($short_low / $exp > )

                $string .= "<td>$short_low</td></tr>";
                $x+=$day_cnt;
            }
            $base = $short_low;
            $str = "";
            ...
            for ($x = 0 ; $x < 100 ; $x++)
            {
                ...
                $short_low = ($lo * $vals[2]);
                $short_low = round($short_low / 100 / 2,2);
                $short_low = $base + $short_low;
                $str .= "<td>$short_low</td></tr>";
                $day_before = round($short_low,2);
                $key = [ ($z), $vals[2], $short_low ];
                $z += $day_cnt;
            }
            $string .= $str . "</table>";
            $string .= "<br>Total Integral";
